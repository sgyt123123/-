...
const InfoPanel: React.FC<InfoPanelProps> = ({ selectedNode, onNavigate }) => {
  ...
  const getTypeConfig = (type: string) => {
      switch(type) {
          case 'cause': return { color: 'text-amber-400', border: 'border-amber-500/50', bg: 'bg-amber-500/10', label: '驱动因素 (DRIVER)' };
          case 'effect': return { color: 'text-rose-400', border: 'border-rose-500/50', bg: 'bg-rose-500/10', label: '社会后果 (EFFECT)' };
          case 'solution': return { color: 'text-emerald-400', border: 'border-emerald-500/50', bg: 'bg-emerald-500/10', label: '破局之道 (SOLUTION)' };
          case 'trap': return { color: 'text-slate-300', border: 'border-slate-500/50', bg: 'bg-slate-500/10', label: '系统陷阱 (TRAP)' };
          default: return { color: 'text-cyan-400', border: 'border-cyan-500/50', bg: 'bg-cyan-500/10', label: '概念 (CONCEPT)' };
      }
  }

  const config = getTypeConfig(selectedNode.type);

  // Selector for Visualizations
  const renderVisualization = () => {
      switch(selectedNode.id) {
          case 'tech_shock': return <TechShockVisual />;
          case 'short_term': return <ShortTermVisual />;
          case 'institution_gap': return <InstitutionGapVisual />;
          case 'resistance': return <ResistanceVisual />;
          case 'prosperity': return <ProsperityVisual />;
          case 'ford_paradox': return <FordParadoxVisual />; 
          case 'trap': return <TrapVisual />;
          default: return null;
      }
  };
    ...
    <div className="h-full flex flex-col border-l border-slate-800 bg-slate-950 shadow-[-20px_0_50px_rgba(0,0,0,0.5)] relative z-30">
      ...
      <div className="p-8 pb-6 bg-slate-950 shadow-xl shrink-0 border-b border-slate-800 relative z-20">
        <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-[10px] font-mono text-slate-600 uppercase tracking-widest">ID: {selectedNode.id.toUpperCase()}</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-8 py-8 custom-scrollbar scroll-smooth bg-slate-950">
        ...
        {/* Dynamic Micro-Visualization Injection */}
        ...
        {/* Causal Navigation Graph Traversal */}
        ...
        {selectedNode.relatedConcepts && selectedNode.relatedConcepts.length > 0 && (
            <div className="mt-8 pt-8 border-t border-slate-800 pb-12">
